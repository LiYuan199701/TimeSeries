...
This section we introduce a new model called Bayesian Nonlinear Expansion Neural Auto-regression Time Series Model. The motivation of designing this model is to help the problems that happen in the last model when the previous model underestimates the high frequency vibration nonlinear time series. As we can observe from the simulation model 4, 5, 6, 7 and their modeled predicted mean, the predicted mean can barely capture the ups and downs of the underlying true time series. As we ponder, we found that all these simulation models have the logarithmic operation which can make a single tanh activation function of a linear combination of lagged time series hard to capture the real pattern. As a result, we think about adding the nonlinear transformations to the each lag including polynomial, absolute value, logarithmic, Fourier features to expand the basis. The essential model specification is for each lag, we take different nonlinear transformations to expand this single lag value into a vector as follows:
$$
...
\end{array}\right]
$$ where $P$ is the maximum number of polynomial degree, $f_1,\cdots,f_m$ are the Fourier frequency we select for the model. The full expansion of lag features is: 
...
\end{array}\right]$$
After this lagged feature is built, we would have our model predicted mean as follows:
$$
